...
local GetConnectedPlayerList = function()
    ...
    for playerIndex = 0, lastPid do
        ...
        if Players[playerIndex] ~= nil and Players[playerIndex]:IsLoggedIn() then
            ...
            list = list .. color.Red .. tostring(Players[playerIndex].name) .. color.Aqua ..
		"  " .. tostring(Players[playerIndex].data.stats.level) .. color.Lavender ..
		"  " .. tostring(Players[playerIndex].data.character.race) .. color.Pink ..
		"  " .. tostring(Players[playerIndex].data.customClass.name) ..
		"\n" .. tostring(Players[playerIndex].data.location.cell) ..
		"  " .. tostring(Players[playerIndex].data.location.regionName) ..
		" (pid: " .. tostring(Players[playerIndex].pid) ..")" .. divider
        end
    end
